...
const payingFetch: typeof fetch = PRIVATE_KEY
  ? (wrapFetchWithPayment(fetch, privateKeyToAccount(PRIVATE_KEY)) as typeof fetch)
  : fetch;

async function callGateway(path: string, body: unknown): Promise<string> {
  ...
  if (!res.ok) throw new Error(`enrich-gate ${path} failed (${res.status}): ${text.slice(0, 300)}`);
  return text;
}
...
server.tool(
  "web_search",
  "Search the web (Google results). Costs $0.01 per call, paid automatically in USDC via x402.",
  { query: z.string().describe("Search query"), num: z.number().optional().describe("Number of results, max 20") },
  async ({ query, num }) => ({ content: [{ type: "text", text: await callGateway("/search", { query, num }) }] })
);

server.tool(
  "neural_search",
  "Semantic/neural web search for conceptual queries (Exa). Costs $0.012 per call via x402.",
  { query: z.string().describe("Natural-language query"), num: z.number().optional() },
  async ({ query, num }) => ({ content: [{ type: "text", text: await callGateway("/neural-search", { query, num }) }] })
);
...
server.tool(
  ...
  async ({ url }) => ({ content: [{ type: "text", text: await callGateway("/scrape", { url }) }] })
);
