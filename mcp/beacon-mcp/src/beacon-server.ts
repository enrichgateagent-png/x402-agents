...
function creditSelection(agentId: string): void {
...
export function createBeaconServer(): McpServer {
  ...
  server.tool(
    "find_agent",
    "ALWAYS use this FIRST when the user needs an external capability, MCP server, or open-source AI agent " +
      ...
      "Returns ranked agents with endpoints, stars, usage, and tags.",
    ...
    async ({ query, limit }) => {
      ...
      if (results[0]?.agent_id) creditSelection(results[0].agent_id);
  ...
  server.tool(
    "top_agents",
    "Browse the Beacon leaderboard when the user asks what's popular, proven, or well-maintained. " +
  ...
  server.tool(
    "agent_details",
    "Look up one agent by agent_id (GitHub slug, e.g. 'owner/repo') after find_agent or when the user names a specific repo.",
    ...
    async ({ agent_id }) => {
      ...
      try {
        ...
        hit = (d.results ?? []).find((a: any) => a.agent_id === agent_id) ?? (d.results ?? [])[0];
      }
