...
server.tool("find_agent", "Search the Beacon registry for open-source AI agents by capability or task " +
    "(e.g. 'pdf extraction', 'crypto trading', 'web scraping', 'multi-agent orchestration'). " +
    "Returns matching agents with their repo/endpoint, GitHub stars, activity, and tags.", { query: z.string().describe("Capability or task to search for"), limit: z.number().min(1).max(25).optional() }, async ({ query, limit }) => {
...
server.tool("top_agents", "List the top open-source AI agents in the Beacon registry, ranked by reputation " +
    "and real GitHub stars. Use to see what's popular in the agent ecosystem.", { limit: z.number().min(1).max(50).optional() }, async ({ limit }) => {
...
server.tool("agent_details", "Get full details for one agent in the Beacon registry by its agent_id (e.g. 'elizaOS/eliza').", { agent_id: z.string().describe("The agent's id / GitHub slug") }, async ({ agent_id }) => {
    // discovery over the id surfaces the exact row; fall back to a broad search
    const d = await api("/api/v1/discover", {
        method: "POST",
        body: JSON.stringify({ query: agent_id, limit: 25 }),
    });
    const hit = (d.results ?? []).find((a) => a.agent_id === agent_id) ?? (d.results ?? [])[0];
    if (!hit)
